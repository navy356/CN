...
int findKeyc(NODEC_PTR *head, char *source, char *dest, uint16_t sport, uint16_t dport)
{
    ...
    while (tmp != NULL)
    {
        ...
        else if ((strcmp(source, tmp->destination) == 0) && (strcmp(dest, tmp->source) == 0) && (tmp->sport == dport) && (tmp->dport == sport) && (tmp->finish == 0))
        {
            return tmp->key;
        }
        tmp = tmp->next;
...
int flow(NODEC_PTR *head, char *source, char *dest, uint16_t sport, uint16_t dport, int conv_key)
{
    NODEC_PTR tmp = getNodec(head, conv_key);
    if(tmp==NULL)
    {
        return -1;
    }
